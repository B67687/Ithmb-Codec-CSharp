...
### Docs
- **AGENTS.md**: Agent-knowledge-base file added — project structure, where-to-look table, conventions, anti-patterns, and commands for AI agent onboarding
...
### Highlights
- Final C# release — complete codec plugin for ImageGlass v10
- All 7 pixel format decoders with SSE2/SSSE3/AVX-512/ARM64 NEON SIMD
- PhotoDB container reader, writer, and integrity checker
- Device-specific format tables for 18 iPod/iPhone generations
- 594 tests, 32 built-in profiles, full cross-platform CI (Linux/macOS/Windows/ARM64)
...
### Highlights
- SSSE3/ARM64 NEON SIMD for CLCL and CL decoders (2.4-3.0× faster)
- REC_RGB555 decoder for iPhone/Touch cover art (Morton Z-order format)
- Full libgpod comparison: all 42 overlapping format IDs verified identical
- 8 new profiles, 54 total
- CI: Linux build, dependabot, benchmark, README stats gate
...
### Highlights
- First real F-prefix .ithmb samples decoded from iPod Classic 6G
- PhotoDB inline JPEG blob detection for unknown-format entries
- Byte-level corruption fuzz test across all 7 decoders
- MaxDecodeFileSize 8 MB → 32 MB after systematic research
- 4 new profiles, benchmark table in README
...
### Highlights
- BGR15 channel-swapped RGB555 for iPhone 2G compatibility
- PhotoDB writer (TryBuildPhotoDb) — write complete ArtworkDB binaries
- PhotoDB integrity checker (IntegrityCheckPhotoDb)
- Device-specific format tables for 18 iPod generations
- Monolithic 6 source files refactored into 15 domain-focused files
...
### Highlights
- Multi-frame raw decode with LRU cache
- CRITICAL: SSE2 buffer overrun and RotateBgra heap overflow fixes
- ARM64 NEON CI (native ubuntu-24.04-arm runners)
- JPEG carving fallback for unknown-prefix files
- Trailing bytes tolerance for real device alignment quirks
...
### Highlights
- Initial release: C# ImageGlass v10 codec plugin for Apple .ithmb thumbnail decoding
- 7 pixel format decoders (RGB565, RGB555, ReorderedRGB555, UYVY, YCbCr420, CLCL, CL)
- Multi-platform SIMD (SSE2, SSSE3, ARM64 NEON) for accelerated decode
- Built-in profiles database (20+ profiles across iPod/iPhone models)
- CLI tools for decode, inspection, and PhotoDB extraction
